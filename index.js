(function(){
  ...
  var Enemy = SheetSprite.extend({
    init: function(clipRects, x, y) {
      ...
      this.stepDelay = 1; // Speed of enemies
    ...
    for (var i = 0, len = bullets.length; i < len; i++) {
      ...
      for (var j = 0, alen = aliens.length; j < alen; j++) {
        ...
        if (checkRectCollision(bullet.bounds, alien.bounds)) {
          ...
          player.score += 100;
          localStorage.setItem("player.credit", parseInt(localStorage.getItem("player.credit")) + 10);
        }
      }
    }
  }
  ...
  function drawBottomHud() {
    ...
    fillText('CREDIT: ' + localStorage.getItem("player.credit") , CANVAS_WIDTH - 115, CANVAS_HEIGHT - 7.5);
